...
void parser_functions(char *comm, char *user, char *canalg) {
  ...
  if(!strcmp(comm, "!!")) {
    ...
    if(!strcmp(comm, "ver")) {
      fprintf(socket_fp, "PRIVMSG %s :%s, Xape BotIrc version %s - by dutra\r\n", canalg, user, VERSION);
      fflush(socket_fp);
    }
    if(!strcmp(comm, "infocs")) {
      ...
      if(comm == NULL) {
	printf("comm\n");
	fprintf(socket_fp, "PRIVMSG %s :%s, You forgot the host, please try again\r\n", canalg, user);
	fflush(socket_fp);
	return;
      }
      ...
      if(comm == NULL) {
	printf("comm\n");
	fprintf(socket_fp, "PRIVMSG %s :%s, You forgot the port, please try again\r\n", canalg, user);
	fflush(socket_fp);
	return;
      }
      port = atoi(comm);
    ...
    if(!strcmp(comm, "wiki")) {
      ...
      if(comm == NULL) {
	printf("comm\n");
	fprintf(socket_fp, "PRIVMSG %s :%s, You forgot the key, please try again\r\n", canalg, user);
	fflush(socket_fp);
	return;		
      }
      strcpy(key, comm);
